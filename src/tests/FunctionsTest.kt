...
class FunctionsTest {
    ...
    fun uniqueCounterTest() {
        ...
        assertEquals(mapOf("a," to 3, "b," to 1, "c," to null), functions.uniqueCounter("a, a, a, b, c", " ")) // Здесь вместо null было 1
    }
    @Test
    fun isPalindromeTest() {
        assertEquals(true, functions.isPalindrome("abccba"))
        assertEquals(true, functions.isPalindrome(" "))
        assertEquals(true, functions.isPalindrome("")) // Здесь было false, но как ни разворачивай пустая строка это по идее тоже палиндром поэтому true
